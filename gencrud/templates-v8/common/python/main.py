...
def registerApi():
    ...
    setattr( API.app,'registerMenu',registerMenu )
    setattr( API.app,'registerSubMenu',registerSubMenu )
    for module in listModules:
        API.app.logger.debug( 'registering module {0}'.format( module ) )
        module.registerApi()

    if API.app.config.get( 'ALLOW_CORS_ORIGIN',False ):
        API.app.logger.info( 'Allowing CORS' )
        if API.app.config.get( 'ALLOW_CORS_ORIGIN',False ):
            origins = API.app.config.get( 'CORS_ORIGIN_WHITELIST','*' )
            API.cors.init_app( API.app, origins = origins )

    logger.info( 'Register Menu route' )
    API.app.register_blueprint( applicApi )
