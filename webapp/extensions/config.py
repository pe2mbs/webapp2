...
from webapp.common.util import lookahead
import yaml
...
def my_safe_load(stream, Loader=yaml.FullLoader, master=None):
    ...
    def _modify( self, c, taskSection = None ):
        ...
        for key in c.keys():
            if key.isupper():
                ...
                if "PATH" in key and c[ key ] is not None and c[ key ].startswith( '.' ):
                    ...
                    if c is not None and c[ key ] is not None and key in delta_keys:
                        if '=' in c[ key ]:
